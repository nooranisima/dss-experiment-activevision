...
K_EXPOSURE = "av:exposure:{item_id}"          # int, times assigned
...
def assign_items(n: int) -> list:
    exposures = []
    for it in ITEMS:
        c = int(r.get(K_EXPOSURE.format(item_id=it["id"])) or 0)
        exposures.append((c, random.random(), it["id"]))
    exposures.sort()
    chosen = [item_id for _, _, item_id in exposures[:n]]
    random.shuffle(chosen)
    for item_id in chosen:
        r.incr(K_EXPOSURE.format(item_id=item_id))
    return chosen
...
def admin_reset_all():
    if not _auth_ok(request):
        return {"error": "unauthorized"}, 403
    deleted = 0
    for pattern in ["av:exposure:*", "av:participant:*", "av:log:*"]:
